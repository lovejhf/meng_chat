...
import com.mengchat.chat.app.MyApp;
import com.mengchat.chat.app.third.location.data.LocationData;
import com.mengchat.chat.kit.audio.AudioPlayManager;
import com.mengchat.chat.kit.audio.IAudioPlayListener;
import com.mengchat.chat.kit.conversation.message.model.UiMessage;
import com.mengchat.chat.kit.conversation.message.viewholder.AudioMessageContentViewHolder;
import com.mengchat.chat.kit.third.utils.UIUtils;
import com.mengchat.chat.kit.utils.DownloadManager;
...
public class ConversationViewModel extends ViewModel implements OnReceiveMessageListener,
    ...
    public void playAudioMessage(UiMessage message) {
        ...
        toPlayAudioMessage = message.message;
            if (message.message.direction == MessageDirection.Receive && message.message.status != MessageStatus.Played) {
                ...
                JSONObject.toJSONString(message.message);
    ...
    public void modifyRedPackageStatus(UiMessage message) {
        if (message == null || !(message.message.content instanceof RedPackgeMessageContent)) {
            return;
        }
    ...
    //发红包
    public void sendRedPackge(RedEnvelopeVo locationData){
        RedPackgeMessageContent rpg = new RedPackgeMessageContent(locationData.getRedTitle(),locationData.getSendLogId(),
                locationData.getSendType(),locationData.getReceiveStatus(),locationData.getRedMoney());
        sendMessage(rpg);
    }
    //红包领取成功发送通知
    public void sendRobRedMessage(String redUid,String receiverId,String fromId){
        RobRedMessageContent robRedMessageContent = new RobRedMessageContent(redUid,receiverId,fromId);
        sendMessage(robRedMessageContent);
    }

    //戳一戳发送通知
    public void sendshakeMessage(String receiverId, String fromId){
        ShakeMessageContent robRedMessageContent = new ShakeMessageContent(receiverId,fromId);
        sendMessage(robRedMessageContent);
    }
